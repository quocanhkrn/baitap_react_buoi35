...
const seatingReducer = (state = initialState, action) => {
  ...
  switch (action.type) {
    case "REJECT_CHOICE":
      choosingStatus = [...state.choosingStatus];
      choosingStatus.forEach((seat, index) => {
        if (seat.seatNo === action.payload) choosingStatus.splice(index, 1);
      });
      state.choosingStatus = [...choosingStatus];
      return { ...state };

    case "CHOOSE":
      const { seatNo, price } = action.payload;
      choosingStatus = [...state.choosingStatus];
      const isChosenIndex = choosingStatus.findIndex((seat) => {
        return seat.seatNo === seatNo;
      });
      if (isChosenIndex === -1) {
        choosingStatus.push({ seatNo, price });
      } else {
        choosingStatus.splice(isChosenIndex, 1);
      }
      state.choosingStatus = [...choosingStatus];
      return { ...state };
